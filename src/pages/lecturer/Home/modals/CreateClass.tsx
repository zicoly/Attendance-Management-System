...
const CreateClassModal = ({ isOpen, onClose, onSubmit }: any) => {
  ...
  const [classDuration, setClassDuration] = useState(60);
  const [locationRequired, setLocationRequired] = useState(false);
  const [allowedRadius, setAllowedRadius] = useState(10); // Changed default to 10m for indoor accuracy
  const [isLoading, setIsLoading] = useState(false);
  const [user, setUser] = useState<any>(null);
    ...
    try {
      ...
      if (locationRequired) {
        try {
          ...
          console.log("Lecturer location:", { latitude, longitude });
          console.log("Set radius:", allowedRadius);
          ...
          geofenceData = {
            enabled: true,
            radius: allowedRadius, // This should be the value from the form
            latitude: latitude,
            longitude: longitude,
            address: address || null,
          };
      ...
      setClassDuration(60);
      setLocationRequired(false);
      setAllowedRadius(5); // Reset to 5m
  ...
  // Helper function to get current position with better accuracy
  const getCurrentPosition = () => {
    return new Promise((resolve, reject) => {
      if (!navigator.geolocation) {
        reject(new Error("Geolocation is not supported by this browser"));
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        },
        {
          ...
          maximumAge: 5000, // Shorter cache time
        }
      );
    });
  };
