...
export default function ClassCard({
  ...
  // Use larger radius to account for GPS inaccuracy
  const allowedRadius = classData.geofence?.radius || 10;
  const classLocation = {
    lat: classData.geofence?.latitude,
    lng: classData.geofence?.longitude,
  };
  ...
  const getLocationWithRetry = async (retries = 3) => {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        console.error(`Location attempt ${i + 1} failed:`, error);
        if (i === retries - 1) {
          setLocationError("Unable to get accurate location");
        }
      }
  ...
  // Enhanced geofence check with multiple criteria
  const isWithinGeofence = () => {
    if (!classData.locationRequired) return true;
    if (!userLocation || !classLocation.lat || !classLocation.lng) return false;

    const gpsWithinRange =
      distanceToClass !== null && distanceToClass <= allowedRadius;

    // If GPS accuracy is poor, be more lenient
    const gpsAccuracy = userLocation.accuracy || 999;
    const adjustedRadius = gpsAccuracy > 10 ? allowedRadius + 5 : allowedRadius;

    const adjustedGpsCheck =
      distanceToClass !== null && distanceToClass <= adjustedRadius;

    console.log("Location check:", {
      distance: distanceToClass,
      allowedRadius,
      gpsAccuracy,
      adjustedRadius,
      gpsWithinRange,
  ...
  const handleMarkAttendance = async () => {
    ...
    if (classData.locationRequired && !isWithinGeofence()) {
      ...
      setAttendanceStatus({
        type: "error",
        message: `Location verification failed. Distance: ${distanceToClass}m (Max: ${allowedRadius}m). GPS accuracy: ${accuracy}m. Try moving closer or reconnecting.`,
      });
      return;
    }
  ...
  const handleCodeSubmit = async () => {
    ...
    if (classData.locationRequired && !isWithinGeofence()) {
      setAttendanceStatus({
        type: "error",
        message: `Final location check failed. Please ensure you're within ${allowedRadius}m of the class.`,
      });
      return;
    }
    ...
    <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
      ...
      <div className="mb-4 p-3 bg-gray-100 rounded-lg text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div>Distance: {distanceToClass}m</div>
          <div>Allowed: {allowedRadius}m</div>
          <div>
            GPS Accuracy:{" "}
            {userLocation?.accuracy ? `${userLocation.accuracy}m` : "Unknown"}
          </div>
          <div>Connection: {connectionInfo?.effectiveType || "Unknown"}</div>
        </div>
        <button
          onClick={() => getLocationWithRetry()}
        ...
        <div className="mb-4">
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium">
                Location Status:{" "}
                {withinGeofence ? "✅ In Range" : "⚠️ Checking..."}
              </span>
            </div>
              ...
              <div className="text-xs space-y-1">
                ...
                {userLocation?.accuracy && userLocation.accuracy > 10 && (
                  <p className="text-orange-600">
                    ⚠️ Poor GPS signal - move outdoors for better accuracy
                  </p>
                )}
              </div>
            )}
